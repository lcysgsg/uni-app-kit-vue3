    ...
    _reload(isClean = false, isFromMounted = false, isUserPullDown = false) {
      ...
      if (!isClean) {
        this._emitQuery(
          ...
          isUserPullDown ? Enum.QueryFrom.UserPullDown : Enum.QueryFrom.Reload,
        );
        let delay = 0;
        // #ifdef MP-TOUTIAO
        delay = 5;
    ...
    _addData(data, success, isLocal) {
      ...
      if (this.isFirstPage) {
        ...
        if (
          this.finalUseCache &&
          success &&
          (this.cacheMode === Enum.CacheMode.Always ? true : this.isSettingCacheList)
        ) {
      ...
      if (success) {
        ...
        if (isLocal) {
          ...
          const localPageSize =
            this.queryFrom !== Enum.QueryFrom.Refresh ? this.defaultPageSize : this.currentRefreshPageSize;
    ...
    _totalDataChange(newVal, oldVal, eventThrow = true) {
      ...
      this.$nextTick(() => {
        ...
        if (
          ...
          this.nIsFirstPageAndNoMore &&
          this.isFirstPage &&
          !this.nFirstPageAndNoMoreChecked
        ) {
